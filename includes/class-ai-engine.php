...
2. Trust Signals (Certs, Awards) — Are there visible trust elements on the page? This includes: certification/award badges, credential logos, client logo bars ('as seen in' / 'trusted by' sections), logo marquees/carousels, case study thumbnails or previews, previous work / portfolio showcase sections, media/partner logo strips, and track-record statements (\"supported over 30 companies\", \"1,000+ projects\"). ANY of these count. [VISUAL but HTML-CONFIRMABLE — IMPORTANT: the screenshot is a FULL-PAGE capture that is heavily downsampled on long pages, so a logo strip or trust section below the hero is often illegible in it. Therefore: (a) if the HTML signal says YES (logo strip/marquee, trust section, or track-record statement detected), set present=true even if you cannot make it out in the screenshot; (b) if the HTML signal says LIKELY, treat it as strong supporting evidence; (c) only mark absent when NEITHER the HTML signal NOR the screenshot shows anything. Never mark absent solely because the downsampled screenshot doesn't clearly show it.]
3. Inline Social Proof — Are there testimonials, review widgets, star ratings, or headshot photos within the body? [VISUAL but HTML-CONFIRMABLE — testimonial/review sections are reliably detectable in the page markup (heading text like \"What our clients say\", or a testimonial/carousel widget). IMPORTANT: (a) if the HTML signal says YES (testimonial section/heading/widget, ratings, or review counts detected), set present=true even if the testimonials are a carousel below the fold that is not legible in the downsampled full-page screenshot; (b) only mark absent when NEITHER the HTML signal NOR the screenshot shows testimonials/reviews/ratings. Never mark absent solely because the downsampled screenshot doesn't clearly show it.]
...
8. SECTION LABELS: Name each rewrite's \"section\" after the page's ACTUAL section — the real heading text or an accurate short description of that block. Do NOT force generic/canonical labels onto sections that don't exist on this page; label only sections that genuinely appear in the CONTENT.{$lead_json_fragment}";
    ...
    private static function call_abacus_ai($prompt, $system_prompt = null, $screenshot_url = null, $max_tokens = 4000)
    {
        ...
        if ($screenshot_url) {
            ...
            $visual_instruction = "A full-page screenshot of this page is attached. It is your primary evidence for ABOVE-THE-FOLD layout and styling.\n\n"
                . "CRITICAL — screenshot limits: this is a FULL-PAGE capture. On long pages it is heavily DOWNSAMPLED, so sections below the hero (logo strips, testimonial carousels, trust badges) are frequently too small to read. Therefore the screenshot is NOT the sole authority for whether a below-fold element EXISTS. And 'UNCONFIRMED FROM HTML' / 'NOT DETECTED' means the HTML parser could not detect the element — NOT that it is absent.\n"
                . "Rule for present=true/false: an element is PRESENT if EITHER the screenshot shows it OR the CRO Structural Signal says YES/LIKELY for it. Only mark present=false when BOTH the screenshot and the HTML signal show nothing. Never mark a below-fold item absent solely because the downsampled screenshot doesn't clearly show it.\n"
                . "- Item 1 (CTA Above the Fold): Is there a clearly styled button or CTA link visible without scrolling?\n"
                . "- Item 2 (Trust Signals): certification/award badges, client logo bars, logo marquees/carousels, 'as seen in' / 'trusted by' strips, case study cards, portfolio images, or a track-record statement ('supported over N companies'). Any one makes present=true. IMPORTANT: if the HTML signal says YES or LIKELY (logo strip/marquee, trust section, or track-record detected), set present=true even if you cannot make it out in the downsampled screenshot.\n"
                . "- Item 3 (Inline Social Proof): testimonial cards, review quotes, headshots, or star-rating widgets. IMPORTANT: if the HTML signal says YES (testimonial section/heading/widget, ratings, or review counts), set present=true even if the testimonials are a carousel below the fold that is not legible in the screenshot. Do NOT mark absent based solely on the screenshot.\n"
