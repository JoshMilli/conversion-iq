...
function conversioniq_run_audit( WP_REST_Request $request ) {
    ...
    foreach ( $pages as $page_id ) {
        ...
        try {
            ...
            $results[] = $ai;
            error_log( '✅ Audit completed for: ' . $post->post_title );
        } catch ( Exception $e ) {
            error_log( '❌ Audit failed for ' . $post->post_title . ': ' . $e->getMessage() );
            // Add fallback result
            $results[] = array(
                'page_id' => $post->ID,
...
function conversioniq_generate_report( WP_REST_Request $request ) {
    ...
    if ( empty( $params['audit_id'] ) ) {
        return new WP_REST_Response( array('success'=>false,'message'=>__('Missing audit_id','conversion-iq')), 400 );
    }
    $audit = ConversionIQ_DB::get_audit( intval( $params['audit_id'] ) );
    if ( ! $audit ) {
        return new WP_REST_Response( array('success'=>false,'message'=>__('Audit not found','conversion-iq')), 404 );
    }
